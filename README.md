...
| | A 原图对照 | B 纯手绘 | C 整页撕纸 |
| --- | --- | --- | --- |
| **成图** | 上半原图，下半纸底拼贴 + 诗 | 整张手绘，无字 | 整页撕纸拼贴，诗在下方纸片上 |
| **尺寸** | 原图 `W:H` → `W:2H`，上下各一半 | 与原图相同 `W:H` | 原图 `W:H` → `W:2H` |
| **例子** | 4:3 → 4:6；16:9 → 16:18 | 16:9 仍是 16:9 | 4:3 → 4:6；16:9 → 16:18 |

诗根据图中要素写。手绘要高度抽象：色块和剪影，不要照片滤镜。C 是一整页撕纸拼贴，不要上下两栏加白线。A 的上半必须是原图像素。
...
```text
使用 $hand-drawn-collage，做版本 A：原图对照。
```
...
```text
使用 $hand-drawn-collage，做版本 C：整页撕纸拼贴。
```
